...
int main(int argc, char* argv[])
{
    ...
    const auto param_cfg = tcfg::ParamConfig::fromCommonCfg(common_cfg);
    ...
    const auto initpath = tcfg::ParamConfig::fmtSrcPath(param_cfg, range.start);
    cv::Mat init = cv::imread(initpath.string());
    state.feed(std::move(init));

    for (int i = range.start; i <= range.end; i++) {
        const auto srcpath = tcfg::ParamConfig::fmtSrcPath(param_cfg, i);
        ...
        for (const auto& mv : state) {
            ...
            const auto dstdir = tcfg::ParamConfig::fmtDstPath(param_cfg, i);
