    ...
    for (const int row : rgs::views::iota(0, arrange_.getMIRows())) {
        for (const int col : rgs::views::iota(0, arrange_.getMICols(row))) {
            ...
            if (arrange_.isKepler()) {
                cv::resize(patch, resizedPatch, {resizedPatchWidth, resizedPatchWidth}, 0, 0, cv::INTER_LINEAR);
            } else {
                cv::rotate(patch, rotatedPatch, cv::ROTATE_180);
                cv::resize(rotatedPatch, resizedPatch, {resizedPatchWidth, resizedPatchWidth}, 0, 0, cv::INTER_LINEAR);
            }

            cv::Mat gradBlendingWeight = cv::Mat::ones({resizedPatchWidth, resizedPatchWidth}, CV_32FC1);
...
#    include "tlct/convert/multiview/xf_merge/impl.cpp"
#endif
