...
public:
    ...
    TLCT_API inline ParamConfig(CalibConfig calib_cfg, int views, cv::Size imgsize, cv::Range range,
    ...
    // Const methods
    [[nodiscard]] TLCT_API inline const CalibConfig& getCalibCfg() const noexcept { return calib_cfg_; };
...
TLCT_API fs::path ParamConfig::fmtSrcPath(const ParamConfig& cfg, int i) noexcept
{
    char buffer[256];
...
TLCT_API fs::path ParamConfig::fmtDstPath(const ParamConfig& cfg, int i) noexcept
{
    char buffer[256];
